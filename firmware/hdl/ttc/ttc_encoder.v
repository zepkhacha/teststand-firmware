...
// Requirements:
//     (1) 'a_channel' is asserted for only one 'clk' cycle
//     (2) 'ttc_data_valid' is asserted for only one 'clk' cycle
...
module ttc_encoder (
  ...
  always @* begin
    ...
    next_ttc_bit_out          = ~ttc_bit_out; // invert by default
  ...
  always @(posedge clk) begin
    // reset state machine
    if (rst) begin
      // never reset the state or output bit
